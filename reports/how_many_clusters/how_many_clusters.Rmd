...
We randomly assign all `nrow(clusters)` clusters to be of either "intervention" or "control" status. The assignments can be seen below.
...
In the above, our methodology works well within Ilha Josina; but at borders, our neighborhoods expand infinitely outwards. In order to account for this, we can trim our polygons to keep only those segments which fall into Magude.
...
The below interactive map allows for manual examination of clusters. Click on any polygon to reveal its cluster number
...
To do this, we first "dissolve" our polygons by intervention status, resulting in the following intrevention zones (red = intervention; green = control).
...
We then re-define our buffers, based on our "dissolved" zones.
...
`ij_clusters` is a projected shapefile with the exact boundaries between clusters
...
The standard shapefile components now exist in our `magude_clusters` directory:

```{r}
dir('magude_clusters/')
```

They are available for download at www.github.com/joebrew/magude_clusters/magude_clusters. Once human inspection confirms the validity of this approach^[Humberto and Bea?], I will make the `magude_clusters` object part of the CISM package.